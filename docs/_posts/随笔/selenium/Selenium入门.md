...
-  ui 测试特征 
-  贴近用户真实使用
  ui 又叫 user interface，是用户直接和软件接触的载体。用户几乎所有的操作都是通过 ui 实现的，因此 ui 测试最能模拟实际的用户使用情况，进行 ui 测试需要站在用户的立场，考虑用户的痛点，模拟用户的行为进行操作。 用户使用产品的功能，是想获得某种能力，因此应该通过功用设计测试用例，而不是单纯的从产品特性和说明来考虑。 
-  变化快 
-  效率低 
